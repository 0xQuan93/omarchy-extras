...
Ui.BarWidget {
  id: root
  Process { id: radioCommand }
  moduleName: "omarchy.media"
  IpcHandler {
    target: "quan-radio"
    function toggle(): void { root.popupOpen = !root.popupOpen }
    function status(): string { return JSON.stringify({ service: !!root.mediaService, title: root.title, popup: root.popupOpen }) }
  }
  ...
  property real maxLabelWidth: 130

  visible: true
  implicitWidth: root.vertical ? root.barSize : Style.space(160)
  ...
  Row {
    ...
    Text {
      ...
      text: root.hasMedia ? root.playIcon : "󰐹"
    ...
    Item {
      ...
      Text {
        id: labelText
        textFormat: Text.PlainText
        text: root.hasMedia ? root.title + (root.artist ? "  ·  " + root.artist : "") : "Quan Radio"
  ...
  MouseArea {
    ...
    onClicked: function(mouse) {
      if (mouse.button === Qt.RightButton) {
        if (root.mediaService && root.activePlayer) root.mediaService.runAction("playPause", false)
      } else if (mouse.button === Qt.MiddleButton) {
        if (root.mediaService && root.activePlayer) root.mediaService.runAction("next", false)
      } else {
    ...
    onEntered: if (root.bar) root.bar.showTooltip(root, root.hasMedia ? (root.title + (root.artist ? " — " + root.artist : "")) : "Quan Radio · click for stations · right-click to play/pause")
  ...
  PopupCard {
    ...
    Column {
      ...
      Text {
        text: "STUDIO FREQUENCIES / SOMAFM"
        color: Color.accent
        font.family: root.bar.fontFamily
        font.pixelSize: Style.font.caption
      }
      Repeater {
        model: [
          { name: "Groove Salad · downtempo", station: "groovesalad" },
          { name: "Drone Zone · ambient", station: "dronezone" },
          { name: "Deep Space One · cosmic", station: "deepspaceone" }
        ]
        Button {
          ...
          foreground: root.bar.foreground
          leftAlign: true
          onClicked: { radioCommand.command = ["/usr/bin/python3", "@DATA@/radio/radio.py", modelData.station]; radioCommand.running = true }
        }
      }
      Row {
        spacing: Style.space(6)
        Button {
          text: "Stop radio"
          foreground: root.bar.foreground
          onClicked: { radioCommand.command = ["/usr/bin/python3", "@DATA@/radio/radio.py", "stop"]; radioCommand.running = true }
        }
        Button {
          text: "SomaFM ↗"
          foreground: root.bar.foreground
          onClicked: {
            ...
            root.bar.run("xdg-open https://somafm.com")
          }
        }
      }
